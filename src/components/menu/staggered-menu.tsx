...
/** SSR-safe layout effect (gsap positions the panel before paint on the client). */
...
export function StaggeredMenu({
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        !toggleBtnRef.current.contains(node)
      ) {
        closeMenu();
      }
    };
  ...
  const layerColors = colors.length ? colors.slice(0, 4) : ["#1e1e22", "#35353c"];
  const prelayers = [...layerColors];
  if (prelayers.length >= 3) prelayers.splice(Math.floor(prelayers.length / 2), 1);
